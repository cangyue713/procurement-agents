...
def _dec(value: float) -> Decimal:
    """float 阈值 -> Decimal（阈值都是整数金额，十进制字面量转换无噪声）。"""
...
def decide_strategy(requirement: Dict[str, Any], rules: RuleConfig | None = None) -> Dict[str, Any]:
    ...
    budget = to_decimal(requirement.get("budget_amount"))  # 状态中可能是 str，统一还原为 Decimal
    urgency = requirement.get("urgency", UrgencyLevel.NORMAL.value)
    rationale: List[str] = []
    applied: List[str] = []
    approval_needed = False

    direct_max = _dec(rules.direct_purchase_max)
    tender_threshold = _dec(rules.tender_threshold)
    ...
    elif budget <= direct_max:
        applied.append("R3")
        rationale.append(f"预算 {budget:,.0f} 元 <= 直接采购上限 {direct_max:,.0f} 元")
        strategy = ProcurementStrategy.DIRECT_PURCHASE
    elif budget > tender_threshold * 5:
        applied.append("R5")
        rationale.append(f"预算 {budget:,.0f} 元 超过公开招标门槛 {tender_threshold * 5:,.0f} 元")
        strategy = ProcurementStrategy.OPEN_TENDER
    elif budget > tender_threshold:
        applied.append("R4")
        rationale.append(
            f"预算 {budget:,.0f} 元 > 招标门槛 {tender_threshold:,.0f} 元，按邀请招标组织"
